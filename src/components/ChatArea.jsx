...
import { MessageSquare, Send, Loader, Check, CheckCheck, Paperclip, X, File, Film, Download } from "lucide-react";
...
const ChatArea = ({ currentUser, teamMembers = [] }) => {
    ...
    const processFile = (file) => {
        ...
        if (file.type.startsWith('image/')) {
            ...
            setFileType('raw'); // PDF, ZIP, RAR, etc.
            setFilePreview(null);
        }
    };
    ...
    const uploadToCloudinary = async (file) => {
        ...
        formData.append("resource_type", "auto"); // IMPORTANT: Lets Cloudinary detect Video vs Image vs Raw
        ...
        try {
            ...
            if (selectedFile) {
                ...
                attachmentData = {
                    ...
                    type: uploadResult.type, // image/video/raw
            ...
            await addDoc(msgsRef, {
                text: newMessage,
                attachment: attachmentData, // Storing object instead of just string
    ...
    const renderAttachment = (msg) => {
        // Backwards compatibility for old messages with just 'imageUrl'
        if (msg.imageUrl && !msg.attachment) {
            return (
                <div className="mb-2 rounded-lg overflow-hidden border border-white/10">
                    <a href={msg.imageUrl} target="_blank" rel="noopener noreferrer">
                        <img src={msg.imageUrl} alt="Attachment" className="max-w-full max-h-60 object-cover" />
                    </a>
                </div>
            );
        }
            ...
            return (
                <div className="mb-2 rounded-lg overflow-hidden border border-white/10">
                    <a href={url} target="_blank" rel="noopener noreferrer">
                        <img src={url} alt="Image" className="max-w-full max-h-60 object-cover hover:scale-105 transition-transform" />
                    </a>
                </div>
            );
        }
        ...
        // Raw File (ZIP, PDF, etc.)
        return (
            <a href={url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 bg-white/10 p-3 rounded-lg border border-white/10 hover:bg-white/20 transition-colors mb-2 group">
                <div className="p-2 bg-blue-500/20 rounded text-blue-400"><File size={20} /></div>
                <div className="flex-1 min-w-0">
                    <div className="text-xs font-bold text-white truncate w-32">{name || "Attached File"}</div>
    ...
    return (
        <div className="flex flex-col h-full bg-[#151518]/30 backdrop-blur-sm relative">
