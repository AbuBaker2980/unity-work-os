...
const TeamView = ({ currentUser, defaultTab = 'roster', onMarkDMRead }) => {
    ...
    // --- UNREAD STATE ---
    const [unreadMap, setUnreadMap] = useState({}); // { [userId]: boolean }
    ...
    useEffect(() => {
        ...
        // When opening a chat, mark as read immediately
        if (selectedChatUser) {
            const storageKey = `dm_last_read_${selectedChatUser.id}`;
            localStorage.setItem(storageKey, new Date().toISOString());

            // Remove from unread map visually immediately
            setUnreadMap(prev => {
                const newMap = { ...prev };
                delete newMap[selectedChatUser.id];
                return newMap;
            });

            // Trigger parent update to clear sidebar Red Dot
            if (onMarkDMRead) onMarkDMRead(selectedChatUser.id);
        }
    }, [selectedChatUser]);

    // --- 2. FETCH MEMBERS & SETUP UNREAD LISTENERS ---
    useEffect(() => {
        if (!currentUser?.teamId) return;

        setLoadingMembers(true);
        // We use onSnapshot for members to get real-time online status updates
        ...
        const unsubscribeMembers = onSnapshot(q, (snapshot) => {
            ...
            if (error.code !== 'permission-denied') console.warn("Members listener fail");
        });

        return () => unsubscribeMembers();
    }, [currentUser.teamId]);

    // --- 3. LISTEN FOR UNREAD MESSAGES (DM) ---
    useEffect(() => {
        ...
        members.forEach(member => {
            ...
            const unsub = onSnapshot(q, (snap) => {
                if (!snap.empty) {
                    ...
                    // Logic: Message exists, I am NOT the sender, and it is newer than my last read time
                    const isUnread = lastMsg.senderId !== currentUser.uid &&
                        lastMsg.createdAt?.toDate().toISOString() > lastRead;
                ...
                if (error.code !== 'permission-denied') console.warn("DM check listener fail");
            });
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(teamRef, (docSnap) => {
            ...
            if (error.code !== 'permission-denied') console.warn("Team name listener fail");
        });
        return () => unsubscribe();
    }, [currentUser.teamId]);
    ...
    const handleLeaveTeam = async () => {
        ...
        try {
            ...
            // App.jsx auth listener will handle the redirect/view change
        } catch (error) {
            console.error(error);
                        ...
                        <div className="flex items-center gap-3 mb-2">
                            {isEditingName ? (
                                <div className="flex items-center gap-2">
                                    <input autoFocus className="bg-[#151518] text-2xl font-bold text-white border border-blue-500/50 rounded-xl px-3 py-1 outline-none w-64" value={editedName} onChange={(e) => setEditedName(e.target.value)} />
                                    <button onClick={handleSaveName} className="p-2 bg-green-600/20 text-green-400 rounded-lg hover:bg-green-600/40"><Check size={18} /></button>
                                    <button onClick={() => setIsEditingName(false)} className="p-2 bg-red-600/20 text-red-400 rounded-lg hover:bg-red-600/40"><X size={18} /></button>
                                </div>
                            ) : (
                                <div className="flex items-center gap-3 group">
                                    <h1 className="text-3xl font-bold text-white tracking-tight">{teamData.name}</h1>
                                    {isTeamLead && <button onClick={() => setIsEditingName(true)} className="opacity-0 group-hover:opacity-100 transition-opacity text-gray-500 hover:text-blue-400" title="Rename Team"><Edit2 size={16} /></button>}
                                </div>
                            )}
                        </div>
                        <div className="flex items-center gap-3">
                            <span className="text-xs font-mono text-gray-500 bg-white/5 px-3 py-1.5 rounded-full border border-white/5 flex items-center gap-2">
                                <Hash size={12} /> {currentUser.teamId}
                            </span>
                            <button onClick={copyInviteCode} className="text-blue-400 text-xs font-bold hover:text-white transition-colors flex items-center gap-1 bg-blue-500/10 px-3 py-1.5 rounded-full border border-blue-500/20 hover:bg-blue-500 hover:border-blue-500"><Copy size={12} /> Invite</button>
                        </div>
                    </div>
                </div>
                        ...
                        {members.map(member => {
                            ...
                            return (
                                <div key={member.id} className={`relative bg-[#121214] border p-5 rounded-3xl transition-all duration-300 hover:shadow-xl hover:-translate-y-1 group flex flex-col items-center text-center
                                    ${hasUnread
                                        ? 'border-blue-500 shadow-[0_0_20px_rgba(59,130,246,0.3)] bg-blue-900/10' // NEON BLUE EFFECT
                                        : isMe ? 'border-blue-500/20 bg-blue-500/5' : 'border-white/5 hover:border-white/10'
                                    }`}>
                                    ...
                                    <div className="relative mb-3">
                                        <div className={`w-16 h-16 rounded-full border-4 overflow-hidden shadow-lg transition-colors ${hasUnread ? 'border-blue-500' : 'border-[#18181b]'}`}>
                                            {member.avatar ? (
                                    ...
                                    <div className="flex gap-2 w-full mt-auto">
                                        {!isMe ? (
                                            ...
                                            // LEAVE TEAM BUTTON FOR NON-LEADS
                                            !isTeamLead && (
        ...
        <div className="flex h-full bg-[#0f0f12] overflow-hidden">
            ...
            <div className="w-72 bg-[#151518] border-r border-white/5 flex flex-col flex-shrink-0 relative z-20 shadow-xl">
                <div className="p-4 border-b border-white/5 shadow-sm bg-[#1a1a1d]">
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="w-full bg-[#0a0a0a] text-xs text-gray-300 pl-9 p-2.5 rounded-xl border border-white/10 outline-none focus:border-blue-500/50 transition-all placeholder-gray-600"
                        />
                    </div>
                </div>
                <div className="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-1">
                    ...
                    >
                        <div className={`p-2 rounded-lg transition-colors ${selectedChatUser === null ? 'bg-white/20' : 'bg-white/5 group-hover:bg-white/10'}`}><Hash size={18} /></div>
                        <div className="text-left">
                            <div className="text-sm font-bold tracking-wide">Team Chat</div>
                            <div className={`text-[10px] ${selectedChatUser === null ? 'text-blue-100' : 'text-gray-600'}`}>General Discussion</div>
                        </div>
                    </button>
                    ...
                    {loadingMembers ? (
                        <div className="space-y-2 p-2">{[1, 2, 3].map(i => <div key={i} className="h-12 bg-white/5 rounded-xl animate-pulse" />)}</div>
                        ...
                        return (
                            <button
                                ...
                                ${selectedChatUser?.id === member.id ? 'bg-white/10 text-white border border-white/5' : 'text-gray-400 hover:bg-white/5 hover:text-gray-200 border border-transparent'}
                                ${hasUnread ? 'border-blue-500 shadow-[0_0_15px_#3b82f6] text-white bg-blue-600/10' : ''} 
                                `}
                            >
                                {selectedChatUser?.id === member.id && <div className="absolute left-0 top-0 bottom-0 w-1 bg-blue-500 rounded-r-full"></div>}

                                <div className="relative">
                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold text-white overflow-hidden border transition-colors ${hasUnread ? 'border-blue-400' : 'border-white/10 bg-[#1e1e21]'}`}>
                                        {member.avatar ? <img src={member.avatar} className="w-full h-full object-cover" /> : member.name?.[0]}
                                    </div>
                                    <div className={`absolute bottom-0 right-0 w-3 h-3 border-2 border-[#151518] rounded-full ${member.isOnline ? 'bg-emerald-500 shadow-[0_0_5px_rgba(16,185,129,0.5)]' : 'bg-gray-500'}`}></div>
                                </div>
                                <div className="text-left flex-1 min-w-0">
                                    <div className={`text-sm font-bold truncate transition-colors ${selectedChatUser?.id === member.id || hasUnread ? 'text-white' : 'text-gray-400 group-hover:text-white'}`}>
                                        {member.name}
                                    </div>
                                    <div className="text-[10px] opacity-50 truncate flex items-center gap-1">
                                        <span className={`w-1.5 h-1.5 rounded-full ${member.role === 'TL' ? 'bg-purple-500' : 'bg-gray-500'}`}></span>
                                        {hasUnread ? <span className="text-blue-300 font-bold">New Message</span> : member.role}
                                    </div>
                                </div>
                                {hasUnread && <div className="w-2.5 h-2.5 rounded-full bg-blue-500 shadow-[0_0_10px_#3b82f6] animate-pulse"></div>}
                            </button>
                        );
                    })}
                </div>
                ...
                <div className="h-16 border-b border-white/5 flex items-center justify-between px-6 bg-[#151518]/80 backdrop-blur-md z-10">
                        ...
                        {selectedChatUser ? (
                            <>
                                <div className="w-10 h-10 rounded-full bg-gray-800 overflow-hidden border border-white/10">
                                    {selectedChatUser.avatar ? <img src={selectedChatUser.avatar} className="w-full h-full object-cover" /> : <span className="w-full h-full flex items-center justify-center text-white text-sm">{selectedChatUser.name[0]}</span>}
                                </div>
                                <div>
                                    <h3 className="font-bold text-white text-base flex items-center gap-2">{selectedChatUser.name}</h3>
                                    <span className={`text-[10px] flex items-center gap-1 font-mono ${selectedChatUser.isOnline ? 'text-emerald-400' : 'text-gray-500'}`}>
                                        <Circle size={6} fill="currentColor" /> {selectedChatUser.isOnline ? 'ONLINE' : 'OFFLINE'}
                                    </span>
                                </div>
                            </>
                        ) : (
                            <>
                                <div className="p-2.5 bg-gradient-to-br from-blue-600 to-indigo-600 rounded-xl text-white shadow-lg shadow-blue-900/20"><Hash size={20} /></div>
                                <div>
                                    <h3 className="font-bold text-white text-base tracking-wide">General Team Chat</h3>
                                    <span className="text-[10px] text-gray-500 font-mono flex items-center gap-1"><Users size={10} /> {members.length} MEMBERS ACTIVE</span>
                                </div>
                            </>
                        )}
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                        <div className="relative flex items-center">
                            {isChatSearchOpen ? (
                                <div className="flex items-center bg-[#0a0a0a] border border-blue-500/50 rounded-lg overflow-hidden animate-slide-left w-48">
                                    <input
                                        autoFocus
                                        className="bg-transparent text-xs text-white p-2 outline-none w-full"
                                        placeholder="Search..."
                                        value={chatSearchText}
                                        onChange={(e) => setChatSearchText(e.target.value)}
                                    />
                                    <button onClick={() => { setIsChatSearchOpen(false); setChatSearchText(""); }} className="p-2 text-gray-500 hover:text-white"><X size={14} /></button>
                                </div>
                            ) : (
                                <button onClick={() => setIsChatSearchOpen(true)} className="text-gray-400 hover:text-white p-2 rounded-lg hover:bg-white/5 transition-colors"><Search size={20} /></button>
                            )}
                        </div>
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-hidden relative bg-[#0a0a0a]">
                    ...
                    {selectedChatUser ? (
                        <GenericChat
                            ...
                            context="direct"
                            collectionPath={['artifacts', 'unity-work-os', 'public', 'data', 'direct_messages', getDirectChatId(currentUser.uid, selectedChatUser.id), 'messages']}
                            queryField={null}
                            queryValue={selectedChatUser.id}
                            currentUser={currentUser}
                            placeholder={`Message @${selectedChatUser.name}...`}
                            filterText={chatSearchText}
                        />
                    ) : (
                        <GenericChat
                            key="global-chat"
