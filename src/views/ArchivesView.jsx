...
import {
    ...
    Download, ChevronRight, CornerUpLeft, Loader, UploadCloud
} from 'lucide-react';
...
const ArchivesView = ({ user }) => {
    ...
    const [isUploading, setIsUploading] = useState(false); // New state for Drag & Drop
    ...
    // --- INTERNAL DRAG (Move Items) ---
    const handleDragStart = (e, itemId) => {
        e.dataTransfer.setData("itemId", itemId);
    };

    // --- MAIN DROP HANDLER (Handles both Internal Moves and External Uploads) ---
    const handleMainDrop = async (e, targetFolderId = currentFolder) => {
        e.preventDefault();
        e.stopPropagation();
        setIsDraggingOver(false);

        // 1. Check if it's an Internal Move (Folder to Folder)
        const internalId = e.dataTransfer.getData("itemId");
        if (internalId) {
            if (internalId === targetFolderId) return;
            try {
                await updateDoc(doc(db, 'artifacts', 'unity-work-os', 'public', 'data', 'archives', internalId), {
        ...
        // 2. Handle External File Drop (OS to App)
        ...
        try {
            ...
            for (const file of files) {
                ...
                await addDoc(collection(db, 'artifacts', 'unity-work-os', 'public', 'data', 'archives'), {
                    title: file.name, // Saves exact filename (e.g. "GameManager.cs")
                    content: textContent,
                    type: 'file',
                    parentFolderId: targetFolderId, // Drops into the specific folder hovered or current
            ...
            let type = activeCategory === 'snippets' ? 'ARCHIVE_SNIPPET_ADDED' : 'ARCHIVE_DOC_ADDED';
            logActivity(`uploaded ${count} files via Drag & Drop`, type);
            toast.success("Files uploaded successfully!", { id: toastId });

        } catch (error) {
            console.error("Upload failed", error);
            toast.error("Failed to upload files. Ensure they are text-based.", { id: toastId });
        } finally {
    ...
    const downloadFile = (title, content, category) => {
        ...
        // If title already has an extension (like from drag & drop), use it.
        // Otherwise, append based on category.
        if (title.includes('.')) {
        ...
        <div className="h-full flex flex-col bg-[#0a0a0a] relative overflow-hidden">
            ...
            <div className="flex flex-1 overflow-hidden relative z-10">
                ...
                <div className="flex-1 flex flex-col min-h-0">
                    ...
                    {/* --- MAIN DROP ZONE --- */}
                    <div
                        className={`flex-1 overflow-y-auto p-6 custom-scrollbar transition-colors ${isDraggingOver ? 'bg-emerald-500/5' : ''}`}
                        onDragOver={handleDragOverMain}
                        onDragLeave={handleDragLeaveMain}
                        onDrop={(e) => handleMainDrop(e, currentFolder)}
                    >
                        ...
                        {isDraggingOver && (
                            <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm pointer-events-none">
                                <div className="bg-[#151518] p-8 rounded-3xl border-2 border-emerald-500 border-dashed animate-bounce flex flex-col items-center">
                                    <UploadCloud size={64} className="text-emerald-500 mb-4" />
                                    <h3 className="text-2xl font-bold text-white">Drop to Upload</h3>
                                    <p className="text-gray-400 mt-2">Files will be added to "{folderPath[folderPath.length - 1].name}"</p>
                                </div>
                            </div>
                        )}
                        ...
                        {visibleFolders.length === 0 && visibleFiles.length === 0 && !searchTerm && (
                            <div className="h-full flex flex-col items-center justify-center text-gray-600 opacity-80 pointer-events-none">
                                ...
                                <p className="text-xs mt-2">Drag and drop files here to upload</p>
                            </div>
                        )}
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4 mb-8">
                                {visibleFolders.map(folder => (
                                    <div
                                        ...
                                        onDrop={(e) => handleMainDrop(e, folder.id)} // Drop directly into specific folder
                                ...
                                >
                                    <div className="flex justify-between items-start mb-3">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <h3 className="font-bold text-gray-200 text-sm truncate w-48" title={item.title}>{item.title}</h3>
